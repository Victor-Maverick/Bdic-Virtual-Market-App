...
interface WebRTCVoiceCallModalProps {
  isOpen: boolean;
  onClose: () => void;
  vendorEmail: string;
  buyerEmail: string;
...
const WebRTCVoiceCallModal: React.FC<WebRTCVoiceCallModalProps> = ({
  isOpen,
  onClose,
  vendorEmail,
  buyerEmail,
  ...
  const {
    ...
    onCallEnd: () => {
      console.log('🎤 WebRTC voice call ended - closing modal');
      if (roomName) {
  ...
  useEffect(() => {
    const cleanup = onCallStatusChange((call) => {
      if (!call || (roomName && call.roomName === roomName &&
        ...
        console.log('🎤 WebRTCVoiceCallModal: Call ended via global context, closing modal');
        onClose();
      }
    });
  ...
  const joinExistingCall = async (existingRoomName: string) => {
    ...
    try {
      ...
      console.error('Error joining WebRTC voice call:', error);
  ...
  useEffect(() => {
    if (isConnected && participants.length > 0) {
      console.log('🎤 Both participants connected - WebRTC voice call is now ACTIVE');
  ...
  const handleEndCall = useCallback(async () => {
    try {
      ...
      if (roomName) {
        ...
        try {
          await voiceCallService.endCall(roomName, userEmail);
        ...
        <div className="flex items-center justify-between p-3 sm:p-4 border-b h-14 sm:h-16 flex-shrink-0">
          <div className="flex items-center space-x-2 sm:space-x-4">
            <h2 className="text-base sm:text-lg font-semibold">
              Voice Call with {isInitiator ? (shopName || vendorEmail.split('@')[0]) : 'Buyer'}
            </h2>
          </div>
        </div>
        ...
        <div className="flex-1 relative bg-gray-50 flex flex-col items-center justify-center">
          ...
          <div className="text-center">
            ...
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">
              {isInitiator ? (shopName || vendorEmail.split('@')[0]) : 'Buyer'}
            </h3>
...
export default WebRTCVoiceCallModal;