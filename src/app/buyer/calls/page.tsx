...
import { twilioCallService, CallResponse } from '@/services/twilioCallService';
...
const BuyerCallsPage: React.FC = () => {
  ...
  const fetchCalls = async () => {
    ...
    try {
      setLoading(true);
      const callsData = await twilioCallService.getCallHistory(session.user.email);
      
      // Filter calls based on selected filter
      let filteredCalls = callsData;
      if (filter !== 'ALL') {
        filteredCalls = callsData.filter(call => call.status === filter);
      }
                    ...
                    <div className="mt-1 sm:mt-2 flex flex-col sm:flex-row sm:items-center sm:space-x-4 gap-1 sm:gap-0 text-xs sm:text-sm text-gray-500">
                      ...
                      {call.answeredAt && call.endedAt && (
                        <div className="flex items-center space-x-1">
                          ...
                          <span>{formatDuration(call.answeredAt, call.endedAt)}</span>
                        </div>
                      )}
                      
                      <span className="capitalize">{call.callType.toLowerCase()} call</span>
                    </div>
                    
                    <div className="mt-1 text-xs sm:text-sm text-gray-600">
                      <p className="truncate">Shop: {call.shopName}</p>
                      {call.productName && (
                        <p className="truncate">About: {call.productName}</p>
                      )}
                    </div>
                  </div>
                </div>
