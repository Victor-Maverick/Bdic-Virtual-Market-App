...
const CallsPage: React.FC = () => {
  ...
  const fetchCalls = async () => {
    ...
    try {
      ...
      const callsData = await twilioCallService.getVendorCalls(session.user.email, status);
      setCalls(callsData);
    } catch (error) {
  ...
  const callBack = async (call: CallResponse) => {
    try {
      const callRequest = {
        ...
        callerName: session?.user?.name || 'Vendor',
        vendorEmail: call.callerEmail,
        vendorName: call.callerName,
